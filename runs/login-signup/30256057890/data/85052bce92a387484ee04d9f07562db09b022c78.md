...
```ts
  4   | /**
  5   |  * Covers /login, /signup ("Enter Account Information"), /account_created,
  6   |  * /logout and /delete_account — one cohesive auth lifecycle.
  7   |  *
  8   |  * Locator strategy per the site research: the /login page's two forms (Login,
  9   |  * and the "New User Signup!" mini-form) are the only place on the whole site
  10  |  * with stable `data-qa` attributes, so those are used directly. The Account
